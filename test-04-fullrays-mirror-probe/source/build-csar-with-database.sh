...
docker build --no-cache . -t proj-eric-oss-drop/fullrays-mirror-probe:$VERSION --build-arg APP_VERSION=$VERSION

# Test-run skipped (you'll test the image manually after the build — see docs)
print_step "Skipping test-run (test the image manually after build)"

# Build CSAR package
print_step "Building CSAR package"

# Create directories
mkdir -p fullrays-mirror-probe
mkdir -p $OUTPUT_DIR

# Copy CSAR directory structure
cp -r ./csar/* ./fullrays-mirror-probe/
...
helm package ./charts/eric-oss-hello-world-python-app/ -d ./fullrays-mirror-probe/OtherDefinitions/ASD/
...
echo "proj-eric-oss-drop/fullrays-mirror-probe:$VERSION" >> $RETAGGED_LIST_FILE
...
MSYS_NO_PATHCONV=1 docker run --init --rm \
  ...
  --workdir /target \
  --volume "$PWD/fullrays-mirror-probe":/target \
...
ls -la $OUTPUT_DIR/fullrays-mirror-probe.csar
if [ ! -f "$OUTPUT_DIR/fullrays-mirror-probe.csar" ]; then
    print_error "CSAR package was not created"
    exit 1
fi

echo -e "${GREEN}=== CSAR package with PostgreSQL support created successfully ===${NC}"
echo "Package location: $OUTPUT_DIR/fullrays-mirror-probe.csar"
