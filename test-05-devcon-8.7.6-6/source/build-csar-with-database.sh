...
# Test-run skipped
print_step "Skipping test-run"

# Build CSAR package
print_step "Building CSAR package"

# Create directories
mkdir -p rapp-devcon-database-demo-app
mkdir -p $OUTPUT_DIR

# Copy CSAR directory structure
cp -r ./csar/* ./rapp-devcon-database-demo-app/
...
helm package ./charts/eric-oss-hello-world-python-app/ -d ./rapp-devcon-database-demo-app/OtherDefinitions/ASD/
...
MSYS_NO_PATHCONV=1 docker run --init --rm \
  ...
  --workdir /target \
  --volume "$PWD/rapp-devcon-database-demo-app":/target \
...
ls -la $OUTPUT_DIR/rapp-devcon-database-demo-app.csar
if [ ! -f "$OUTPUT_DIR/rapp-devcon-database-demo-app.csar" ]; then
    print_error "CSAR package was not created"
    exit 1
fi

echo -e "${GREEN}=== CSAR package with PostgreSQL support created successfully ===${NC}"
echo "Package location: $OUTPUT_DIR/rapp-devcon-database-demo-app.csar"
